    ...
    public void addToMap() {
        ...
        try {
            ...
            for (int i = 0; i < walkPaths.size(); i++) {
                ...
                LatLng latLng = Utils.convertToLatLng(latLonPoints.get(0));
                
				addWalkStationMarkers(walkStep, latLng);
				for(LatLonPoint point:walkStep.getPolyline()){
				    mPolylineOptions.add(Utils.convertToLatLng(point));
                }
//                addWalkPolyLines(walkStep);
    ...
    private void addWalkPolyLine(LatLonPoint pointFrom, LatLonPoint pointTo) {
        addWalkPolyLine(Utils.convertToLatLng(pointFrom), Utils.convertToLatLng(pointTo));
    }
    ...
    private void addWalkPolyLines(WalkStep walkStep) {
        mPolylineOptions.addAll(Utils.convertArrList(walkStep.getPolyline()));
    }
