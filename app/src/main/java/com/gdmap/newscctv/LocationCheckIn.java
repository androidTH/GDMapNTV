...
public class LocationCheckIn extends AppCompatActivity implements AMap.OnCameraChangeListener, AMap.OnMapLoadedListener, AMapLocationListener, PoiSearch.OnPoiSearchListener, View.OnClickListener {
    ...
    private void init() {
        ...
        searchResultAdapter = new SearchResultAdapter(LocationCheckIn.this);
    ...
    public void onCameraChangeFinish(CameraPosition cameraPosition) {
        ...
        if (mcircle != null) {
            if (mcircle.contains(cameraPosition.target)){
                ...
                Toast.makeText(LocationCheckIn.this, "微调距离不可超过500米", Toast.LENGTH_SHORT).show();
                aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(mlocation ,16f ));
            }
        }else {
            startLocation();
            Toast.makeText(LocationCheckIn.this, "重新定位中。。。", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void checkin() {
        if (checkinMarker  == null ){
            if (checkinpoint != null) {
                ...
                Toast.makeText(LocationCheckIn.this, "签到成功", Toast.LENGTH_SHORT).show();
            } else{
                startLocation();
                Toast.makeText(LocationCheckIn.this, "请定位后重试，定位中。。。", Toast.LENGTH_SHORT).show();
            }
        }else {
            Toast.makeText(LocationCheckIn.this, "今日已签到", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void onPoiSearched(PoiResult poiResult, int resultCode) {
        ...
        if (resultCode == AMapException.CODE_AMAP_SUCCESS){
            if (poiResult != null && poiResult.getPois().size() > 0){
                ...
                Toast.makeText(LocationCheckIn.this, "无搜索结果", Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(LocationCheckIn.this, "搜索失败，错误 "+resultCode, Toast.LENGTH_SHORT).show();
        }
    }
}
